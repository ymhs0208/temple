...
function LockedMedal() {
  return <div className="locked-medal-placeholder" aria-hidden="true"><span>?</span><small>解鎖後顯示</small></div>;
}
...
export default function BadgesPage() {
  ...
  return <main id="badge-collection">
    ...
    <header className="collection-hero">
      ...
      <div className="collection-showcase" aria-hidden="true"><span className="orbit orbit-one"/><span className="orbit orbit-two"/>{ready && earned(catalog[5]) ? <Medal badge={catalog[5]}/> : <LockedMedal/>}<span className="showcase-caption">完成挑戰後，勳章會在這裡發光</span></div>
    </header>
    ...
    <section className="collection-grid" aria-label="學習徽章列表" aria-busy={!ready}>
      {visible.map(b => <button key={b.id} className={`collection-card ${b.metric} ${earned(b) ? "earned" : "locked"}`} onClick={() => open(b)} disabled={!ready} aria-label={`${b.title}，${earned(b) ? "已解鎖" : "未解鎖"}，查看詳情`}><div className="collection-card-top"><span>{ranks[b.rank]}</span><span>{earned(b) ? "✓ 已解鎖" : "未解鎖"}</span></div>{earned(b) ? <Medal badge={b}/> : <LockedMedal/>}<h3>{b.title}</h3><p>{b.target} {unit(b.metric)}</p><progress value={Math.min(progress[b.metric], b.target)} max={b.target} aria-label={`${b.title}解鎖進度`}/><small>{earned(b) ? "已收藏 · 點我欣賞" : `${progress[b.metric]} / ${b.target} · 查看挑戰`}</small></button>)}
    </section>
    ...
    <dialog ref={dialog} className="collection-dialog" onClose={() => { setSelected(null); setPlaying(false); }} aria-labelledby="collection-dialog-title">
      {selected && <div className={`collection-dialog-body ${selected.metric} reveal-${badgeDesigns[selected.metric][selected.rank][4]} ${playing ? "is-celebrating" : ""}`} style={{ "--reveal-color": badgeDesigns[selected.metric][selected.rank][2], "--reveal-edge": badgeDesigns[selected.metric][selected.rank][3] } as CSSProperties} key={`${selected.id}-${replay}`}>
        ...
        <div className="collection-stage"><div className="collection-burst"/>{playing && earned(selected) && <div className="collection-confetti" aria-hidden="true">{Array.from({length:24}, (_,i) => <i key={i} style={{"--i": i} as CSSProperties}/>)}</div>}{earned(selected) ? <Medal badge={selected}/> : <LockedMedal/>}</div>
