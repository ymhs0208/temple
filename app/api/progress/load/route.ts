...
export async function POST(request: Request) {
  try {
    ...
    const { data: plan, error: planError } = await db.from("study_plans").select("id, exam_date, daily_hours, weak_subject, goal, challenge_name, wishes").eq("user_id", user.id).maybeSingle();
    ...
    return Response.json({ exists: true, plan: { challengeName: plan.challenge_name, examDate: plan.exam_date, hours: Number(plan.daily_hours), weak: plan.weak_subject, goal: plan.goal, wishes: Array.isArray(plan.wishes) ? plan.wishes : [] }, tasks: (rows ?? []).map((row, index) => ({ subject: row.subject, minutes: row.minutes, detail: row.task_type, done: complete.has(row.id), color: colors[index % colors.length] })), energy, visits: visits?.map((row) => row.temple_code) ?? [] });
  } catch (error) {
