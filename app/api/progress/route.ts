...
export async function POST(request: Request) {
  try {
    const body = await request.json() as { idToken?: string; hours?: number; weak?: string; goal?: string; challengeName?: string; examDate?: string; tasks?: IncomingTask[] };
    ...
    const examDate = /^\d{4}-\d{2}-\d{2}$/.test(body.examDate ?? "") ? body.examDate! : new Date(Date.now() + 29 * 86400000).toISOString().slice(0, 10);
