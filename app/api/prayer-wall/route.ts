...
async function userForToken(idToken?: string) {
  ...
  return {
    db,
    userId: user.id,
    displayName: identity.displayName ?? "學習夥伴",
  };
}
export async function GET() {
  try {
    ...
    const { data, error } = await db
      ...
      .select("id, display_name, message, is_anonymous, created_at")
    ...
    const featured = posts.length ? posts[seed % posts.length] : null;
    return Response.json({ posts, featured, featuredDate: dayKey });
  } catch {
  ...
  try {
    ...
    if (body.action === "report") {
      ...
      const { data: post, error } = await db
        ...
        .maybeSingle();
      if (error || !post)
        return Response.json({ error: "Post unavailable" }, { status: 404 });
      const reports = post.report_count + 1;
      await db
        .from("prayer_wall_posts")
        .update({
          report_count: reports,
          moderation_status: reports >= 3 ? "hidden" : "published",
        })
        .eq("id", body.postId);
      return Response.json({ ok: true, hidden: reports >= 3 });
    }
    const message = clean(body.message ?? "");
    if (message.length < 2 || message.length > 120)
      return Response.json(
        { error: "Message must be 2–120 characters" },
        { status: 400 },
      );
    ...
    const needsReview =
      prohibited.some((word) => message.includes(word)) ||
    ...
    const { error } = await db
      ...
      .insert({
        user_id: userId,
        display_name:
          body.anonymous !== false ? "匿名學習夥伴" : displayName.slice(0, 20),
        message,
        is_anonymous: body.anonymous !== false,
        moderation_status: needsReview ? "pending" : "published",
      });
    if (error) throw error;
