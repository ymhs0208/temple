...
type StoredPlan = { tasks?: Task[]; challengeName?: string; goal?: string; hours?: number; weak?: string; taskAdjustmentCounts?: { deferred: number; split: number; skipped: number } };
...
const taipeiDateFromDate = (date: Date) => new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Taipei" }).format(date);
...
export default function StatisticsPage() {
	...
	useEffect(() => {
		try {
			...
			if (stored.hours) setDailyGoalHours(stored.hours);
			if (stored.weak) setWeakSubject(stored.weak);
	...
	const weeklyReport = useMemo(() => {
		...
		const weekStart = taipeiDateFromDate(monday);
		const records = learningRecords.filter((record) => record.date >= weekStart && record.date !== today);
		const thisWeekTasks = [...records.flatMap((record) => record.tasks), ...tasks];
		const completedTasks = thisWeekTasks.filter((task) => task.done).length;
		const completionRate = thisWeekTasks.length ? Math.round((completedTasks / thisWeekTasks.length) * 100) : 0;
		const pendingBySubject = new Map<string, number>();
		thisWeekTasks.filter((task) => !task.done).forEach((task) => pendingBySubject.set(task.subject, (pendingBySubject.get(task.subject) ?? 0) + 1));
		if (weakSubject) pendingBySubject.set(weakSubject, (pendingBySubject.get(weakSubject) ?? 0) + 1);
		const stuckSubject = [...pendingBySubject.entries()].sort((a, b) => b[1] - a[1])[0]?.[0] ?? (weakSubject || "尚未判定");
		const weeklyTargetMinutes = dailyGoalHours * 7 * 60;
		const focusRate = weeklyTargetMinutes ? Math.min(100, Math.round((displayedWeeklyMinutes / weeklyTargetMinutes) * 100)) : 0;
		const advice = completionRate < 60
			? `下週先把每天的任務縮成 1–2 件，優先完成「${stuckSubject}」的 25 分鐘複習。`
			: focusRate < 70
				? `下週每天安排 ${Math.max(15, Math.ceil((weeklyTargetMinutes - displayedWeeklyMinutes) / 7 / 5) * 5)} 分鐘專注時段，先從「${stuckSubject}」開始。`
				: `本週節奏很穩定；下週保留既有安排，並為「${stuckSubject}」多加一個錯題整理時段。`;
		const adjustmentNote = taskAdjustments.deferred + taskAdjustments.skipped > 0 ? ` 本週有 ${taskAdjustments.deferred} 次延後、${taskAdjustments.split} 次拆分、${taskAdjustments.skipped} 次跳過；下週先替「${stuckSubject}」預留較短的專注時段。` : "";
		return { completionRate, focusRate, stuckSubject, advice: `${advice}${adjustmentNote}`, weeklyTargetMinutes };
	}, [dailyGoalHours, displayedWeeklyMinutes, learningRecords, tasks, today, weakSubject, taskAdjustments]);
