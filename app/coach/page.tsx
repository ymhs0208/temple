  ...
  return (
    <main className="feature-page coach-page">
      <section className="feature-shell">
        ...
        <section className="coach-data-status" aria-live="polite"><b>個人化資料</b><p>{dataStatus}</p><button disabled={loading || dataLoading} onClick={() => setRefresh(value => value + 1)}>重新讀取</button><a href="/goal">設定目標 →</a>{!idToken && !dataLoading && <a href="/profile">前往登入 →</a>}<p>詢問時會將學習摘要交給 AI 產生建議；登入憑證不會傳給 AI。</p></section>
        <section className="coach-context" aria-label="目前學習情境">
          <span>距離目標 <b>{daysLeft ?? "未設定"}</b>{daysLeft !== null ? " 天" : ""}</span>
        ...
        <section className="coach-evidence" aria-label="教練使用的今日資料">
				...
				<section className="coach-proactive" aria-label="每日主動學習服務">
					...
					<div>
						...
						<p>{(cloudCounts?.due ?? dueWeakQuestions.length) ? `目前有 ${cloudCounts?.due ?? dueWeakQuestions.length} 題錯題需要回流，建議先從少量錯題開始複習。` : remaining.length ? `今天還有 ${remaining.length} 項任務，先從最重要的一項開始。` : "目前沒有待辦任務，可前往今日頁面確認計畫。"}</p>
					</div>
					<button onClick={() => void askCoach("請根據今天的學習資料，安排我現在最適合開始的下一步。")} disabled={loading || dataLoading || dataError}>{loading ? "安排中…" : "開始學習"}</button>
				</section>

        <section className="coach-setup" aria-label="本次學習設定">
          <div className="coach-section-heading"><div><span>01 / 調整步調</span><h2>現在的你，適合怎麼讀？</h2></div><a href="/goal">調整學習目標 ↗</a></div>
        ...
        <section className="coach-today">
          <div><b>今日尚未完成</b><small>{remaining.length ? `${remaining.length} 項任務等待你開始` : "目前沒有待辦任務，可前往今日頁面確認計畫"}</small></div>
        ...
        <section className="coach-history">
          <div className="coach-section-heading"><div><span>02 / 留下方向</span><h2>最近的軍師建議</h2></div>{history.length > 0 && <button onClick={() => { setHistory([]); try { if (!idToken) localStorage.removeItem("wenchang-coach-history"); } catch { /* Storage may be unavailable; keep the current in-memory state. */ } }}>清除紀錄</button>}</div>
