...
export default function Home({ initialTab = "today" }: { initialTab?: Tab }) {
	...
	const today = (
		<>
			<section className="hero">
				<p className="eyebrow">☀ 一次一小步，今天一起進步</p>
				<h1>
					{pendingIndex >= 0 ? `今天還有 ${tasks.length - completed} 個任務` : "今天任務已完成"}
				</h1>
				<div className="countdown">
					<span>{pendingIndex >= 0 ? `下一個：${tasks[pendingIndex].subject}・${tasks[pendingIndex].minutes} 分鐘` : `今日已完成 ${completed}/${tasks.length} 項任務`}</span>
				</div>
			...
			<section className="today-command-card" aria-label="下一個學習任務">
				...
				<button onClick={startFocus} disabled={pendingIndex < 0}>{pendingIndex >= 0 ? `開始 ${tasks[pendingIndex].minutes} 分鐘專注` : "今日已完成"}</button>
			</section>
			<section id="today-todos" className="progress-card" aria-labelledby="today-todos-title">
				<div className="section-heading">
					<div>
						<p className="eyebrow">MY DAILY CHECKLIST</p>
						<h2 id="today-todos-title">今日待辦</h2>
						<p className="todo-instruction">勾選記錄完成，點任務開始專注。</p>
					</div>
				...
				<div className="task-overview" aria-label="今日任務摘要">
					...
					<span>
						...
						<details className="today-task-list" open>
									<summary>任務列表 · {tasks.filter(task => !task.done && !task.skipped).length} 項待完成</summary>
					<div className="tasks">
					{tasks.map((task, index) => (
						...
						>
							...
							<span className={`subject-dot ${task.color}`} aria-hidden="true">{String(index + 1).padStart(2, "0")}</span>
											<button
	...
	const progressView = (
		<section className="journey progress-workspace">
			<p className="eyebrow">學習進度</p>
			<h1>
				今天完成了 {completed} 項任務
				<br />
				<em>距離目標還有 {daysLeft} 天</em>
			</h1>
			...
			<section id="prayer-wish" className="wish-card">
				...
				<div className="wish-composer">
					<input
						...
						maxLength={40}
					/>
					<button onClick={saveWish} disabled={!wish.trim()}>儲存願望</button>
				</div>
				{wishes.length === 0 && <p className="prayer-empty">還沒有願望。寫下一件今天想做到的小事吧！</p>}
				{wishes.length > 0 && (
		...
		<section className="journey profile-page" id="account-settings">
			...
			<div className="account-list">
				...
				<section className="weekly-care-card" aria-label="每週關懷摘要">
					<div><h2>每週學習摘要</h2><p>一週的努力，一次看懂。</p><small>包含完成率、專注時間與待加強科目，不顯示題目內容。</small></div>
					<div className="care-audience" role="group" aria-label="摘要版本">
