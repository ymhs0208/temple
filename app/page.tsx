...
const getCountdownPhase = (daysLeft: number): CountdownPhase => {
    ...
    return {
        ...
        title: "穩定累積",
        label: "長線準備",
        detail: "先穩定完成，再逐步提高弱科與歷屆題的比重。",
        factor: 1,
        weakRatio: 0.45,
        pastRatio: 0.32,
    };
};
const buildCountdownTasks = (
    weak: string,
    hours: number,
    ...
    const total = Math.max(
        45,
        Math.round((Math.max(1, hours) * 60 * phase.factor) / 5) * 5,
    );
    const weakMinutes = Math.max(
        15,
        Math.round((total * phase.weakRatio) / 5) * 5,
    );
    const pastMinutes = Math.max(
        15,
        Math.round((total * phase.pastRatio) / 5) * 5,
    );
...
const culturalScenes = [
    {
        ...
        seal: "學",
        title: "書院門前",
        detail: "完成第一段完整專注，替今天立下學習的起點。",
    },
    {
        days: 3,
        seal: "初",
        title: "青木書院",
        detail: "連續三天以完成任務回應自己，木牌正式點亮。",
    },
    {
        days: 7,
        seal: "穩",
        title: "晨鐘長廊",
        detail: "七日穩定累積，讓規律成為可以依靠的節奏。",
    },
    {
        days: 14,
        seal: "進",
        title: "燈火書齋",
        detail: "半月精進，回望錯題與弱點，讓理解逐漸清晰。",
    },
    {
        days: 30,
        seal: "願",
        title: "文昌殿",
        detail: "三十日真實完成，解鎖專屬祈願場景與回顧時刻。",
    },
] as const;
...
export default function Home({ initialTab = "today" }: { initialTab?: Tab }) {
    ...
    const [draftNotificationKinds, setDraftNotificationKinds] =
        useState(notificationKinds);
    ...
    useEffect(() => {
        if (hydrated && !localStorage.getItem("wenchang-onboarding-complete"))
    ...
    const onboardingSteps = [
        {
            icon: "✓",
            title: "今日",
            detail: "安排今天的學習任務，開始專注並直接勾選完成。",
        },
        {
            icon: "▥",
            title: "進度",
            detail: "查看專注時間、完成率與最近的學習趨勢。",
        },
        {
            icon: "✦",
            title: "祈福",
            detail: "完成簽到、求一支學習籤，也能到匿名祈福牆留下祝福。",
        },
        {
            icon: "⛩",
            title: "巡禮",
            detail: "到合作宮廟掃描 QR Code，探索故事並收藏文化碎片。",
        },
    ] as const;
    ...
    useEffect(() => {
        ...
        const showSleepReminderIfDue = () => {
            ...
            if (
                time >= "22:30" &&
                localStorage.getItem(SLEEP_REMINDER_KEY) !== today
            ) {
        ...
        if (stored)
            try {
                ...
                if (typeof data.oracleTickets === "number")
                    setOracleTickets(
                        data.oracleWelcomeGranted
                            ? data.oracleTickets
                            : data.oracleTickets + 1,
                    );
                if (!data.oracleWelcomeGranted) {
                    localStorage.setItem(
                        ...
                        JSON.stringify({
                            ...
                            oracleTickets: (data.oracleTickets ?? 0) + 1,
                        }),
                    );
                }
    ...
    const unlockedSceneIndex = culturalScenes.reduce(
        (latest, scene, index) =>
            checkInStreak >= scene.days ? index : latest,
        0,
    );
    ...
    const minutesToNextPlank =
        focusRewardMinutes % 10 === 0 ? 10 : 10 - (focusRewardMinutes % 10);
            ...
            setWeakQuestions((current) =>
                current.map((entry) =>
                    entry.id === item.id
                        ? {
                              ...entry,
                              reviewStep: 2 as const,
                              nextReviewDate: addTaipeiDays(
                                  entry.firstWrongDate,
                                  3,
                              ),
                          }
                        : entry,
    ...
    const toggleTask = (index: number) => {
        ...
        setSyncStatus(
            task.done
                ? `「${task.subject}」已恢復為未完成。`
                : `「${task.subject}」已直接標記完成。`,
        );
    };
    const beginFocus = () => {
    ...
    const abandonFocus = async () => {
        if (
            !(await confirmAction({
                title: "要先離開專注嗎？",
                message:
                    "這次倒數尚未完成。任務會保留，但本次計時將結束，之後可以重新開始。",
                confirmLabel: "保留任務並離開",
                cancelLabel: "繼續專注",
            }))
        )
            return;
    ...
    const chartMaxMinutes = Math.max(
        30,
        ...weeklyChartData.map((day) => day.minutes),
    );
    const todayTaskEntries = tasks
        .map((task, index) => ({ task, index }))
        .sort(({ task: a }, { task: b }) => {
            const rank = (item: Task) =>
                item.done
                    ? 3
                    : item.skipped
                      ? 4
    ...
    const today = (
        <>
            <header className="simple-today-heading">
                <div>
                    <p>今日學習</p>
                    <h1>
                        {tasks.length === 0
                            ? "從一個小計畫開始"
                            : pendingIndex >= 0
                              ? `今天還有 ${tasks.length - completed} 個任務`
                              : completed === tasks.length
                                ? "今天的任務都完成了"
                                : "今天沒有待辦任務"}
                    </h1>
                    <span>
                        {name} · 距離目標 {daysLeft} 天
                    </span>
                </div>
                <a href="/goal">調整計畫</a>
            </header>
            <section className="simple-next-task" aria-label="下一個學習任務">
                <div>
                    <span>{pendingIndex >= 0 ? "建議先完成" : "下一步"}</span>
                    <h2>
                        {pendingIndex >= 0
                            ? tasks[pendingIndex].subject
                            : tasks.length === 0
                    ...
                    <p>
                        {pendingIndex >= 0
                            ? `先做這一項，完成後再做下一項。${tasks[pendingIndex].detail}`
                            : "可以查看學習紀錄，或調整今天的安排。"}
                    </p>
                </div>
                {pendingIndex >= 0 ? (
                    <button onClick={startFocus}>
                        開始 {tasks[pendingIndex].minutes} 分鐘專注 →
                    </button>
                ) : (
                    <a
                        href={
            ...
            {completionFeedback && (
                <section
                    className="task-completion-feedback"
                    aria-live="polite"
                >
                    <div className="task-completion-heading">
                        <span>✓</span>
                        <div>
                            <small>剛剛完成</small>
                            <h2>{completionFeedback.subject}完成</h2>
                        </div>
                        <button
                            aria-label="關閉完成回饋"
                    ...
                    <div className="task-completion-metrics">
                        <div>
                            <b>
                                {completionFeedback.completedCount}
                                <small>
                                    {" "}
                                    / {completionFeedback.totalCount}
                                </small>
                            </b>
                            <span>今日完成</span>
                        </div>
                        <div>
                            <b>
                                {completionFeedback.remainingMinutes}
                                <small> 分鐘</small>
                            </b>
                            <span>剩餘時間</span>
                        </div>
                    ...
                    <div className="task-completion-actions">
                        ...
                        <button onClick={() => setCompletionFeedback(null)}>
                            休息一下
                        </button>
                        <a href="/progress">查看今日成果</a>
                    </div>
                </section>
            )}
            <section
                ...
                aria-labelledby="today-todos-title"
            >
                ...
                <div className="task-overview" aria-label="今日任務摘要">
                    ...
                    <span>
                        ...
                        <b>
                            {tasks
                                .filter((task) => !task.done && !task.skipped)
                        ...
                        {todayTaskEntries.map(({ task, index }) => (
                            ...
                            >
                                ...
                                >
                                    ...
                                    <small
                                        className={`task-status ${task.done ? "is-done" : task.skipped ? "is-skipped" : focusIndex === index ? "is-active" : "is-pending"}`}
                                    >
                                        {task.done
                                            ? "已完成"
                                            : task.skipped
                                ...
                                {adjustingTaskIndex === index && !task.done && (
                                    <div className="task-adjust-menu">
                                        <button
                                            onClick={() => deferTask(index)}
                                        >
                                            延後到明天
                                        </button>
                                        <button
                                            onClick={() => splitTask(index)}
                                        >
                                            拆成 15 分鐘
                                        </button>
            ...
            <section className="today-ai-entry" aria-label="AI 學習教練入口">
                <div className="today-ai-entry-mark" aria-hidden="true">
                    ✦
                </div>
                <div>
                    <span>不知道先做哪個？</span>
                    <strong>請 AI 幫你安排</strong>
                    <p>告訴我還有幾分鐘，我會排出今天的下一步。</p>
                </div>
                <div className="today-ai-entry-actions">
                    <a href="/coach">告訴我時間</a>
                    <small>也可以在 LINE 傳「我只有一小時」</small>
                </div>
            </section>
            <section
                className="today-pilgrimage-recommendation"
                aria-label="文昌巡禮推薦"
            >
                <div
                    className="today-pilgrimage-recommendation-icon"
                    aria-hidden="true"
                >
                    ⛩
                </div>
                ...
                <a href="/pilgrimage">
                    開始文昌巡禮 <span aria-hidden="true">→</span>
                </a>
            </section>

            <details className="simple-study-note">
                <summary>學習目標與提醒</summary>
                <div>
                    <p>
                        <b>我的目標</b>
                        {goal}
                    ...
                    <p>
                        ...
                        {countdownPhase.detail}
                    </p>
                    <button onClick={() => setSleepReminderOpen(true)}>
                        查看今晚休息提醒
                    </button>
                    <a href="/progress">查看倒數階段建議 →</a>
                </div>
            </details>
            {syncStatus && (
                <p className="simple-sync-status" role="status">
                    {syncStatus}
                </p>
            )}
        </>
        ...
        <section className="journey progress-workspace" id="learning-progress">
            <header className="progress-page-heading">
                <div>
                    ...
                    <h1>學習進度</h1>
                    <span>
                        {name} · 距離目標還有 {daysLeft} 天
                    </span>
                </div>
                <a href="/today">回到今日任務 →</a>
            </header>
            <section className="progress-overview" aria-label="學習概況">
                <div className="progress-today-stat">
                    <span>今日任務</span>
                    <strong>
                        {completed}
                        <small> / {tasks.length} 項</small>
                    </strong>
                    <progress
                        value={completed}
            ...
            <section className="progress-next-step">
                <div>
                    ...
                    <h2>
                        {pendingIndex >= 0
                            ? `繼續完成「${tasks[pendingIndex].subject}」`
                    ...
                    <p>
                        {pendingIndex >= 0
                            ? `預計 ${tasks[pendingIndex].minutes} 分鐘，回到今日即可開始。`
                            : "可以回顧學習紀錄，或調整下一步的目標。"}
                    </p>
                </div>
                <a href="/today">
                    {pendingIndex >= 0 ? "繼續學習 →" : "查看今日任務 →"}
                </a>
            </section>
            <section className="progress-charts" aria-label="學習數據圖表">
                <article className="progress-chart-card">
                    <header>
                        ...
                        <b>{displayedWeeklyMinutes} 分鐘</b>
                    </header>
                    <div
                        className="weekly-bar-chart"
                        role="img"
                        aria-label={`最近七天共學習 ${displayedWeeklyMinutes} 分鐘`}
                    >
                        {weeklyChartData.map((day) => (
                            <div
                                className="weekly-bar-item"
                                key={day.key}
                                title={`${day.key}：${day.minutes} 分鐘`}
                            >
                                <div className="weekly-bar-track">
                                    <i
                                        style={{
                                            height: `${day.minutes ? Math.max(8, (day.minutes / chartMaxMinutes) * 100) : 0}%`,
                                        }}
                                    />
                                </div>
                    ...
                    <p>柱子越高，代表當天完成的學習時間越多。</p>
                </article>
                <article className="progress-chart-card progress-completion-chart">
                    <header>
                        <div>
                            <span>今天</span>
                            <h2>任務完成多少？</h2>
                    ...
                    <div
                        ...
                        style={
                            { "--completion": `${progress}%` } as CSSProperties
                        }
                    >
                        <strong>
                            {completed}
                    ...
                    <p>
                        {tasks.length
                            ? `還有 ${Math.max(0, tasks.length - completed)} 項任務待完成。`
                            : "先建立今日學習計畫。"}
                    </p>
                </article>
            </section>
            <div className="progress-content-grid">
                    ...
                    >
                        <div className="weakness-heading">
                            <div>
                                <span>到期錯題複習</span>
                                <b>
                                    今天有 {dueWeakQuestions.length} 題可以複習
                                </b>
                            </div>
                ...
                <aside
                    className="progress-side-column"
                    aria-label="計畫與更多紀錄"
                >
                    <section className="progress-tools">
                        ...
                        <a href="/goal">
                            ...
                            <div>
                                <b>調整學習計畫</b>
                                <small>修改目標日期、科目與每日時間</small>
                            </div>
                            <span aria-hidden="true">›</span>
                        </a>
                    </section>
                    <details className="progress-plan-details">
                        <summary>
                            <div>
                                <b>倒數階段建議</b>
                                <small>
                                    {countdownPhase.label} · {daysLeft} 天
                                </small>
                            </div>
                            <span>展開</span>
                        </summary>
                        <section
                            className={`countdown-plan-card countdown-${countdownPhase.id}`}
                            aria-label="考試倒數計畫"
                        >
                            <div className="countdown-plan-heading">
                                <div>
                                    <span>
                                        考試倒數計畫・{countdownPhase.label}
                                    </span>
                                    <b>{countdownPhase.title}</b>
                                </div>
                                <strong>
                            ...
                            <div className="countdown-plan-footer">
                                <span>
                                    今日安排 {countdownTotalMinutes}{" "}
                                    分鐘・弱科優先{" "}
                                    {Math.round(countdownPhase.weakRatio * 100)}
                                    %
                                </span>
                                <button
                                    onClick={applyCountdownPlan}
    ...
    const quickActions = (
        <section className="quick-actions">
            <div className="quick-title">
                <div>
                    <b>文昌七媽學堂怎麼幫你</b>
                    <small>從安排學習到完成回顧，所有功能集中在這裡</small>
                </div>
                <span>6 個學習工具</span>
            </div>
            <div className="feature-highlight-strip" aria-label="產品功能流程">
                <div>
                    <b>① 今天照著做</b>
                    <span>AI 依弱科與可用時間排出任務</span>
                </div>
                <div>
                    <b>② 完成就有回饋</b>
                    <span>專注、勾選、徽章與學習籤同步累積</span>
                </div>
                <div>
                    <b>③ LINE 會提醒你</b>
                    <span>收到今日任務、下一步與每週摘要</span>
                </div>
            </div>
        ...
        <section id="prayer-simple" className="journey prayer-workspace">
                ...
                {dailyFortuneTask.done && (
                    ...
                    >
                        <div className="daily-small-step-copy">
                            ...
                            <div>
                                <span>
                                    簽到後的今日一小步・{dailySmallStep.minutes}{" "}
                                    分鐘
                                </span>
                                <b>{dailySmallStep.title}</b>
                                <small>
                                    {dailySmallStep.detail}{" "}
                                    完成紀錄請以專注任務計時為準。
                                </small>
                            </div>
                        </div>
            ...
            {checkInCeremonyOpen && (
                <div className="checkin-ceremony" role="presentation">
                    ...
                    >
                        <div
                            className="checkin-ceremony-rays"
                            aria-hidden="true"
                        />
                        <p className="checkin-ceremony-kicker">今日修習留存</p>
                        <div
                            className="checkin-ceremony-seal"
                            aria-hidden="true"
                        >
                            <span>✓</span>
                        </div>
                        ...
                        <div className="checkin-ceremony-step">
                            <span>
                                接著做一小步・{dailySmallStep.minutes} 分鐘
                            </span>
                            <b>{dailySmallStep.title}</b>
                        </div>
                        <p className="checkin-ceremony-date">
                            願你把這份專注，帶進今天的每一段學習。
                        </p>
                        <button
                            onClick={() => setCheckInCeremonyOpen(false)}
                            autoFocus
                        >
                            收下祝福
                        </button>
            ...
            <section id="prayer-wish" className="wish-card">
                ...
                >
                    <span className="wall-link-icon" aria-hidden="true">
                        ✦
                    </span>
                    <div>
                        <b>探索匿名祈福牆</b>
                ...
                <small className="wall-safety-note">
                    請保持匿名，不要留下姓名、電話、地址或其他個人資料。
                </small>
            </section>
            <section
                id="prayer-oracle"
                    ...
                    <div className="oracle-exchange">
                        ...
                        <div>
                            ...
                            <p className="oracle-plank-guide">
                                木牌取得方式：完成完整專注任務，每 10 分鐘獲得 1
                                枚。
                                {minutesToNextPlank > 0
                                    ? ` 再專注 ${minutesToNextPlank} 分鐘可獲得下一枚。`
                                    : " 下一枚木牌已準備好。"}
                            </p>
                            <p>
                                收錄 {fortunePoems.length}{" "}
                                支學習鼓勵籤，送給正在努力的你。原創勵志內容，不作成績或吉凶預測。
                            </p>
                            <p>
                                每 3 枚木牌可兌換 1
                            ...
                            {oracleTickets > 0 && (
                                <button
                                    className="oracle-welcome-button"
                                    onClick={startWelcomeOracle}
                                >
                                    首次贈送・免費求一籤
                                </button>
                            )}
            ...
            <section
                className="learning-culture-bridge"
                aria-labelledby="learning-culture-bridge-title"
            >
                <div className="learning-culture-bridge-heading">
                    <span aria-hidden="true">⛩</span>
                    <div>
                        <small>為什麼放在學習平台？</small>
                        <h2 id="learning-culture-bridge-title">
                            文化體驗，最後都回到下一步學習
                        </h2>
                    </div>
                </div>
                <p>
                    祈福不是另外一個遊戲，而是把完成任務後的鼓勵，轉成明確的學習行動。
                </p>
            ...
            <details className="prayer-details prayer-explore">
                ...
                <section
                    className="cultural-reward-card"
                    aria-label="文化化的學習回饋"
                >
                    <div className="cultural-reward-heading">
                        <div className="cultural-scene-seal" aria-hidden="true">
                            {activeCulturalScene.seal}
                        </div>
                        <div>
                    ...
                    <div className="cultural-proof">
                        <div>
                            <b>{focusSessionsCompleted}</b>
                            <span>次完整專注</span>
                        </div>
                        <div>
                            <b>{focusRewardMinutes}</b>
                            <span>分鐘真實累積</span>
                        </div>
                        <div>
                            <b>{checkInStreak}</b>
                        ...
                        {culturalScenes.slice(1).map((scene) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div>
                                        <b>
                                            {scene.days} 日・{scene.title}
                                        </b>
                                        <span>
                                            {unlocked
                                                ? "已由真實完成解鎖"
                                                : `還需連續學習 ${scene.days - checkInStreak} 天`}
                                        </span>
                                    </div>
                                </li>
                            );
                    ...
                    <p className="cultural-reward-note">
                        {nextCulturalScene
                            ? `下一個場景：${nextCulturalScene.title}。完成完整專注任務、留下今日學習紀錄，讓場景隨習慣自然開展。`
                            : "所有祈願場景皆已由你的真實學習完成解鎖。"}
                    </p>
                </section>
    ...
    const saveNotifications = async () => {
        if (
            !draftMorningTime ||
            !draftEveningTime ||
            draftMorningTime === draftEveningTime
        ) {
    ...
    const profileView = (
        <section className="journey profile-page" id="account-settings">
            ...
            <section
                className="proactive-service-card"
                aria-labelledby="proactive-service-title"
            >
                <div className="proactive-service-heading">
                    <span aria-hidden="true">🔔</span>
                    <div>
                        <p>LINE 主動學習服務</p>
                        <h2 id="proactive-service-title">
                            每天提醒你下一步，不必等到想起來才開始
                        </h2>
                    </div>
                </div>
                ...
                <p className="proactive-service-note">
                    只會傳送給你的
                    LINE，不會直接通知老師或家長；可在下方自由開關。
                </p>
            </section>
            <section className="notification-preference">
                    ...
                    <div className="notification-status-actions">
                        ...
                        {lineName && !editingNotifications && (
                            <button
                                ...
                                onClick={
                                    lineName ? beginEditNotifications : login
                                }
                            >
                                {lineName ? "修改" : "登入後設定"}
                            </button>
                ...
                {!lineName ? (
                    <>
                        ...
                        <button className="notification-login" onClick={login}>
                    ...
                    <>
                        ...
                        <div className="notification-times">
                            <fieldset
                                className="line-notification-kinds"
                                disabled={
                                    !draftRemindersEnabled ||
                                    savingNotifications
                                }
                            >
                                ...
                                {(
                                    ...
                                    <label key={key}>
                                        <input
                                            type="checkbox"
                                            checked={
                                                draftNotificationKinds[key]
                                            }
                                            onChange={(e) =>
                                                setDraftNotificationKinds(
                                                    (current) => ({
                                                        ...current,
                            ...
                            <label>
                                ...
                                <input
                                    ...
                                    onChange={(event) =>
                                        setDraftMorningTime(event.target.value)
                                    }
                                />
                            </label>
                            <label>
                                ...
                                <input
                                    ...
                                    disabled={!draftRemindersEnabled}
                                    onChange={(event) =>
                                        setDraftEveningTime(event.target.value)
                                    }
                                />
                            </label>
                        </div>
                    ...
                    <div className="notification-summary">
                        <div>
                            ...
                            <b>
                                {remindersEnabled &&
                                notificationKinds.morningEnabled
                                    ? formatReminderTime(morningTime)
                                    : "已關閉"}
                            </b>
                        </div>
                        <div>
                            ...
                            <b>
                                {remindersEnabled &&
                                notificationKinds.eveningEnabled
                                    ? formatReminderTime(eveningTime)
                                    : "已關閉"}
                            </b>
                        </div>
                        <div>
                            ...
                            <b>
                                {remindersEnabled &&
                                notificationKinds.weeklyEnabled
                                    ? `週日 ${formatReminderTime(eveningTime)}`
                                    : "已關閉"}
                            </b>
                        </div>
                    </div>
                ...
                <small className="notification-timezone">
                    台灣時間・這三類自動通知每天最多兩則。週日回顧取代晚間提醒；需加入
                    LINE 官方帳號好友，並同步學習紀錄。
                </small>
                {lineName && !notificationPreferencesReady && (
                    <p role="status">
            ...
            <section className="app-shell">
                ...
                {tab === "today" ? (
                    <section id="today-simple" aria-label="今日學習工作區">
                        {today}
                        <details className="simple-more-tools">
                            <summary>
                                其他功能 <small>進度、徽章、教練與祈福</small>
                            </summary>
                            {quickActions}
                            <a
                                className="simple-line-link"
                    ...
                    <div className="onboarding-backdrop" role="presentation">
                        ...
                        >
                            <div
                                ...
                                aria-label={`引導第 ${onboardingStep + 1} 步，共 ${onboardingSteps.length} 步`}
                            >
                ...
                {focusIndex !== null && tasks[focusIndex] && (
                    ...
                    >
                        ...
                        <div className="focus-session-content">
                            ...
                            <div
                                className="focus-noise-control"
                                aria-label="白噪音控制"
                            >
                                <button
                                    className={focusNoiseEnabled ? "is-on" : ""}
                                    onClick={() =>
                                        setFocusNoiseEnabled(
                                            (enabled) => !enabled,
                                        )
                                    }
                                    aria-pressed={focusNoiseEnabled}
                                >
                                    <span aria-hidden="true">◌</span> 白噪音{" "}
                                    {focusNoiseEnabled ? "開啟中" : "關閉"}
                                </button>
                                {focusNoiseEnabled && (
                                    <label>
                                        音量
                ...
                {sleepReminderOpen && (
                    ...
                    >
                        ...
                        >
                            ...
                            <p>
                                讓大腦好好休息。睡得夠，明天才能把今天讀過的內容真正記住。
                            </p>
                            <button onClick={() => setSleepReminderOpen(false)}>
                                知道了，準備收心
                            </button>
                        </section>
                    </div>
                ...
                {unlockCelebration && (
                    ...
                    >
                        ...
                        >
                            <div
                                className="badge-unlock-sparkles"
                                aria-hidden="true"
                            >
                                ✦　✧　✦
                            </div>
                            <span className="badge-unlock-kicker">
                                NEW ACHIEVEMENT
                            </span>
                            <div
